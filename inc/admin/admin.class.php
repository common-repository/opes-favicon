...
class _OFWP_jvet__Admin {

	private function __construct( $params ) {
		//global _OFWP_jvet__Conf::$added_PHP_Admin_Files;

		if ( is_array( _OFWP_jvet__Conf::$added_PHP_Admin_Files ) ) {
			foreach ( _OFWP_jvet__Conf::$added_PHP_Admin_Files as $filePath ) {
				include_once( $filePath );
			}
		}

		$PHP_ToLoad = scandir( __OFWP_jvet__THIS_PLUGIN__ADMIN_DIR_ . 'controller' . __OFWP_jvet__PS_ );
		$PHP_ToLoad = preg_grep( '/-ToLoad\.php$/i' , $PHP_ToLoad );
		sort ( $PHP_ToLoad , SORT_NUMERIC );

		foreach ( $PHP_ToLoad as $key => $fileName ) {
			include( __OFWP_jvet__THIS_PLUGIN__ADMIN_DIR_ . 'controller' . __OFWP_jvet__PS_ . $fileName );
		}
/*		
		$directory = new RecursiveDirectoryIterator( __OFWP_jvet__THIS_PLUGIN__ADMIN_DIR_ . 'controller/' );
...
*/
	...
	public static function init( $params ) {
		return new _OFWP_jvet__Admin( $params );
	}

	public function addAddedScriptsAndStyles( $hook ) {
		//global _OFWP_jvet__Conf::$added_SCRIPT_Admin_Files , _OFWP_jvet__Conf::$added_STYLE_Admin_Files;

		if ( is_array( _OFWP_jvet__Conf::$added_SCRIPT_Admin_Files ) ) {
			foreach ( _OFWP_jvet__Conf::$added_SCRIPT_Admin_Files as $script ) {
				if ( isset( $script[ 'hook_deps' ] ) ) {
					...
					print_r( _OFWP_jvet__Main::addScript( $script ) );
				}
			}
		};

		if ( is_array( _OFWP_jvet__Conf::$added_STYLE_Admin_Files ) ) {
			foreach ( _OFWP_jvet__Conf::$added_STYLE_Admin_Files as $style ) {
